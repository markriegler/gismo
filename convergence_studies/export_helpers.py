...
class AdditionalBlocks:
    ...
    def add_boundary_conditions(
        ...
        """
        # Check if at least boundary conditions or a corner value is prescribed
        if function_list is None and bc_list is None and cv_list is None:
            raise ValueError(f"Neither boundary conditions nor corner values are prescribed. Provide at least one of them.")
        ...
        if cv_list is not None:
            for cv_unknown, cv_patch, cv_corner, cv_function in cv_list:
                function_dict_cv = {
                    "tag": "cv",
                    "attributes": {
                        "unknown": str(cv_unknown),
                        "patch": str(cv_patch),
                        "corner": str(cv_corner),
                    },
                    "text": cv_function,
                }
